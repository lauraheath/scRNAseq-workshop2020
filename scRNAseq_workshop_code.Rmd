...
Repeat with another Subcluster: Ex6
...
```{r}
up_In9 <- subset(fit_coefs2, fit_coefs2$test_val>0)
top_up_In9 <- up_In9 %>%
                top_n(-6, q_value)
genelist_up_In9 <- unique(top_up_In9 %>% pull(gene_short_name))
plot_cells(cds_subset, genes=genelist_up_In9,cell_size=1,label_cell_groups=0,show_trajectory_graph=FALSE)
```
...
```{r}
down_In9 <- subset(fit_coefs2, fit_coefs2$test_val<0)
top_down_In9 <- down_In9 %>%
                top_n(-6, q_value)
genelist_down_In9 <- unique(top_down_In9 %>% pull(gene_short_name))
plot_cells(cds_subset, genes=genelist_down_In9,cell_size=1,label_cell_groups=0,show_trajectory_graph=FALSE)
```
...
```{r}
In9_up_plotgenes <- cds_subset[rowData(cds_subset)$gene_short_name %in% genelist_up_In9,]
plot_genes_in_pseudotime(In9_up_plotgenes, color_cells_by="Diagnosis",min_expr=0.5)
```


```{r}
In9_down_plotgenes <- cds_subset[rowData(cds_subset)$gene_short_name %in% genelist_down_In9,]
plot_genes_in_pseudotime(In9_down_plotgenes, color_cells_by="Diagnosis",min_expr=0.5)
```
